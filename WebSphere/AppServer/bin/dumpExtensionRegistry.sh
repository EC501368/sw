...
SHELL=com.ibm.ws.runtime.DumpExtensionRegistry
...
"$JAVA_HOME"/bin/java \
  ...
  $JVM_EXTRA_CMD_ARGS \
  -classpath "$C_PATH" \
  com.ibm.wsspi.bootstrap.WSPreLauncher -nosplash -application com.ibm.ws.bootstrap.WSLauncher $SHELL "$CONFIG_ROOT" "$WAS_CELL" "$WAS_NODE" "$@"
