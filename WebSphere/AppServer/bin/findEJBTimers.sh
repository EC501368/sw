...
# EJB Timer Service Find EJB Timers Command Launcher

# Launch Arguments:
#
# findEJBTimers.sh <server> <filter> [options]
# server  : the name of the server process to look for timers.
# filter: <-all> | <-timer timerid> | <-app [-mod [-bean]]>
#         -all                        : find all timers
#         -timer <timerid>            : find a specific timer only
#         -app <application name>     : find all timers for an applicaiton
#         -mod <module name>          : find all timers for a module
#         -bean <bean name>           : find all timers for an EJB type
# options:
...
# Bootstrap values ...
SHELL=com.ibm.ws.admin.services.FindEJBTimersCommand
