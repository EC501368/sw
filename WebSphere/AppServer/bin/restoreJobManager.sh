...
# JobManager Database Restore Tool

#Arguments:
# location - location of the backup database e.g c:/foo/bar also includes the database name in the string.


APP_EXT_ID=com.ibm.ws.admin.system.JobManagerRestore
...
#   $APP_EXT_ID "$USER_INSTALL_ROOT" "$@"  $javaOption \
...
"$JAVA_HOME"/bin/java \
  ...
  $APP_EXT_ID "$USER_INSTALL_ROOT" "${nonJavaOption[@]}"
