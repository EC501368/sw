...
D_ARGS="-Dws.ext.dirs="$WAS_EXT_DIRS" $DELIM -Dwas.install.root="$WAS_HOME" $DELIM -Djava.endorsed.dirs="$WAS_ENDORSED_DIRS" $DELIM -Djava.util.logging.manager=com.ibm.ws.bootstrap.WsLogManager $DELIM -Djava.util.logging.configureByServer=true "$OSGI_INSTALL" "$OSGI_CFG""
...
${JAVA_EXE} \
  ...
  $JVM_EXTRA_CMD_ARGS \
  -classpath "$C_PATH" \
  com.ibm.wsspi.bootstrap.WSPreLauncher -nosplash -application com.ibm.ws.bootstrap.WSLauncher $SHELL revoke "$@"
