...
executeWRD() {
	...
	$JAVA_HOME/bin/java -Djava.endorsed.dirs="$WAS_ENDORSED_DIRS" $ENCODINGOPTIONS -Xms256m -Xmx512m -cp $wrd_cp org.eclipse.core.launcher.Main -noupdate -data "$WORKSPACE" -application $APPLICATION -config "$@"
}
