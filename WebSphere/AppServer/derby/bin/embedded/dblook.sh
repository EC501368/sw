...
"$JAVA_HOME/bin/java" $CONSOLE_ENCODING -Dwas.install.root="$WAS_HOME" -Dderby.system.home="$DERBY_HOME" $X_ARGS $D_ARGS -classpath "$WAS_CLASSPATH:$DERBY_HOME/lib/derby.jar:$DERBY_HOME/lib/derbytools.jar:$DERBY_HOME/lib/jh.jar:$__DERBY_LOCALE_JARS_PATH__" com.ibm.ws.bootstrap.WSLauncher org.apache.derby.tools.dblook "$@"
