#!/bin/sh
# ---------------------------------------------------------
#-- This batch file is an example of how to run networkServerControl command
...
"$JAVA_HOME/bin/java" $CONSOLE_ENCODING -Dwas.install.root="$WAS_HOME" $X_ARGS $D_ARGS -classpath "$WAS_CLASSPATH:$DERBY_HOME/lib/derbyclient.jar:$DERBY_HOME/lib/derby.jar:$DERBY_HOME/lib/derbytools.jar:$DERBY_HOME/lib/derbynet.jar:$DERBY_HOME/lib/jh.jar:$__DERBY_LOCALE_JARS_PATH__" -Dderby.system.home="$DERBY_HOME" com.ibm.ws.bootstrap.WSLauncher org.apache.derby.drda.NetworkServerControl "$@"
